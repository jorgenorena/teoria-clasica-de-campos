...
Ahora consideremos transformaciones que involucran las coordenadas:
\[
  x'^\mu=x^\mu+\epsilon^a\delta_ax^\mu,
  \qquad
  \phi_i'(x')=\phi_i(x)+\epsilon^a\delta_a\phi_i(x).
\]

Definimos la variación en un punto
\[
  \Delta\phi_i(x)=\phi_i'(x)-\phi_i(x)=
\]
\end{document}
