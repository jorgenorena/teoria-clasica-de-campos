...
\[
  \hat i=(\cos\theta,-\sin\theta),
  \qquad
  \hat j=(\sin\theta,\cos\theta),
\]
es decir,
\[
...
\begin{align*}
  \vec v
  &=v_x\hat i+v_y\hat j \\
  &=\left(v_x\cos\theta+v_y\sin\theta\right)\hat i'
   +\left(v_y\cos\theta-v_x\sin\theta\right)\hat j'.
\end{align*}
...
\[
  v_x'^2+v_y'^2
  =v_x^2(\cos^2\theta+\sin^2\theta)
   +v_y^2(\cos^2\theta+\sin^2\theta)
  =v_x^2+v_y^2.
\]
...
\begin{align*}
  ...
  &=\cos\theta\,\mathbb{1}+i\sin\theta\,J \\
  &=
  \begin{pmatrix}
...
Este grupo se llama $SO(2)$.

Un grupo similar es el de los complejos unimodulares $|z|=1$, llamado $SU(1)$:
\[
  |z_1z_2|=|z_1||z_2|=1,
  \qquad |1|=1,
  \qquad |z^{-1}|=\frac1{|z|}=1,
